...
var (
	...
	repo = r.NewRepository(
		...
		r.NewAdoOrganization(&db),
		r.NewAdoOrganizationApprovalRequest(&db),
		r.NewApprovalRequest(&db),
		r.NewApprovalType(&db),
		r.NewApprover(&db),
		r.NewCommunityApprover(&db),
	...
	serv = s.NewService(
		...
		s.NewAdoOrganizationService(repo),
		s.NewAdoOrganizationApprovalRequestService(repo),
		s.NewApprovalRequestService(repo),
		s.NewApprovalTypeService(repo),
		s.NewApproverService(repo),
		s.NewCommunityApproverService(repo),
	...
	cont = c.NewController(
		...
		c.NewAdoOrganizationController(serv, conf),
