CREATE PROCEDURE [dbo].[usp_Search]
  ...
  @OffSet [INT] = 0,
  @Filter [INT] = 0,
  @SelectedSourceType [VARCHAR] (100) = NULL,
...
BEGIN
  ...
  SELECT *
  INTO [#TemporaryCombinedResults]
  FROM [CombinedResults] WHERE [Source] = @SelectedSourceType OR @SelectedSourceType IS NULL;

  SELECT
    *
  FROM
    [#TemporaryCombinedResults]
  ORDER BY [Score] DESC
  OFFSET @OffSet ROWS
